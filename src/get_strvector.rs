...
//                          add the mod date values 
                         if baddate1 == 0 {
                             let dateto = Utc.with_ymd_and_hms(dateyr, datemo, dateday, datehr.try_into().unwrap(), datemin.try_into().unwrap(), datesec.try_into().unwrap()).unwrap();
                             let dateto = dateto + 
                                                   Duration::days(dateyrx*365) +
...
//                                     let mut datetox = Utc.ymd(2000,1,1).and_hms_milli(1,1,1,0);
                                     ...
                                     let mut datetox = Utc.with_ymd_and_hms(dateyr, datemo, dateday, datehr.try_into().unwrap(), datemin.try_into().unwrap(), datesec.try_into().unwrap()).unwrap();
